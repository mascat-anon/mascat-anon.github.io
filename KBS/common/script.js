...
function 表示() {
	for (let j = 0; j < 5; j++) { //初期状態にもどしてく
		...
		document.getElementById('Remark' + j).innerHTML = "";
	...
	var DispTableCount = 0;
	if (TodayTimeTable.length >= 5) {
		DispTableCount = 5;
	} else {
	...
	for (let i = 0; i < DispTableCount; i++) {
		...
		RemarkBunData.forEach((index) => {
			if (index[0] == TodayTimeTable[i][4]) {
				document.getElementById('Remark' + i).innerHTML = index[1];
      ...
      document.getElementById('Time' + i).innerHTML = "時刻未定"
		...
		if (TodayTimeTable[i][0] == "当日") { //当日データが取れているものだけ吐く
			...
			if (TodayTimeTable[i][8] == "far") { 
				...
				document.getElementById('Remark' + i).innerHTML =  position + "つ前のバス停を出発。" +
				TodayTimeTable[i][6] + "には" + TodayTimeTable[i][7] + "ごろ到着予定。"; //上書き
			}
...
const timerId = setInterval(function(){//表示成功まで繰り返す
	if (ScrollForceParam == 1) {
		document.getElementById('Message').innerHTML = ScrollForceMessage;
		CheckEmpty = ScrollForceMessage;
	  } else if (ScrollCommonParam == 0) {
			document.getElementById('Message').innerHTML = ScrollInfoMessage;
			CheckEmpty = ScrollInfoMessage;
		} else if (ScrollCommonParam == 1) {
			document.getElementById('Message').innerHTML = TrainInfoMessage;
